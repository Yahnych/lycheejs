...
lychee.define('game.entity.Level').requires([
	...
	Class.prototype = {
		...
		__deserialize: function(model) {
			...
			for (var s = 0, sl = settings.length; s < sl; s++) {
				...
				for (var b = 0, bl = raw.buildings.length; b < bl; b++) {
					...
					if (bmodel !== null) {
						...
						var building = new bconstr(playerdata.id, {
							model: bmodel,
				...
				for (var u = 0, ul = raw.units.length; u < ul; u++) {
					...
					if (umodel !== null) {
						...
						var unit = new uconstr(playerdata.id, {
							model: umodel,
				...
				this.__players[playerdata.id] = playerdata;
			...
			// Automized Player Selection, Multiplayer Map
		...
		__updateHitmap: function(id, entity) {
			...
			if (layerHitmap !== null) {
				...
				if (hitmap !== null) {
					...
					var x1 = Math.floor(position.x - size.x / 2);
					var y1 = Math.floor(position.y - size.y / 2);
					var x2 = Math.round(position.x + size.x / 2);
					var y2 = Math.round(position.y + size.y / 2);
