...
lychee.define('tool.API').tags({
	...
	let _WHITESPACE = new Array(513).join(' ');

	let _readable_params = function(params) {
	...
	let _readable_values = function(properties, force) {
	...
	let _readable_types = function(raw) {
	...
	let _readable_type = function(raw) {
	...
	let _dynamic_type = function(raw) {
	...
	let _dynamic_value = function(raw) {
	...
	let _parse_enums = function(code) {
	...
	let _parse_events = function(code) {
	...
	let _parse_properties = function(code) {
		...
		code.split('\n').forEach(function(line) {
			...
			if (line.substr(0, 7) === '\t\tthis.' && line.indexOf('=') !== -1) {
				...
				if (key.substr(0, 2) !== '__') {
					...
					val = _dynamic_value(value);
					typ = _dynamic_type(value);
	...
	let _parse_methods = function(code) {
		...
		code.split('\n').filter(function(line) {
			...
			if (line.substr(0, 2) === '\t\t' && line.indexOf(': function') !== -1) {
				...
				if (typ !== undefined && val !== undefined) {
					...
					let param = params.find(function(value) {
	...
	const Composite = function(identifier, code) {
